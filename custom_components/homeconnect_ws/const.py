"""Constants."""

from __future__ import annotations

from typing import Final

from homeassistant.const import Platform

DOMAIN: Final = "homeconnect_ws"
PLATFORMS: Final = [
    Platform.BINARY_SENSOR,
    Platform.SENSOR,
    Platform.SWITCH,
    Platform.SELECT,
    Platform.BUTTON,
    Platform.NUMBER,
]

CONF_PSK: Final = "psk"
CONF_AES_IV: Final = "aes_iv"
CONF_FILE: Final = "file"
CONF_MANUAL_HOST: Final = "manual_host"
CONF_DEV_SETUP_FROM_DUMP: Final = "setup_from_dump_enabled"
CONF_DEV_OVERRIDE_HOST: Final = "override_host"
CONF_DEV_OVERRIDE_PSK: Final = "override_psk"
APPLIANCE_HANDSHAKE_TIMEOUT: Final = 180
APPLIANCE_SEND_TIMEOUT: Final = 45
OAUTH_TOKEN_KEEPALIVE_INTERVAL: Final = 30 * 24 * 60 * 60
OAUTH_TOKEN_RETRY_INTERVAL: Final = 24 * 60 * 60
SOCKET_CONNECT_TIMEOUT: Final = 5
SOCKET_RECONNECT_DELAY: Final = 1
SOCKET_PING_INITIAL_DELAY: Final = 5
SOCKET_PING_INTERVAL: Final = 30
